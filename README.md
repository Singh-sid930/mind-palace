...
**[singh-sid930.github.io/mind-palace](https://singh-sid930.github.io/mind-palace/)**, nothing to install. (The Gemma companion stays home; everything else travels.)
...
attention's score; the singular values become LoRA's rank. The top floor, vision-language-action, is reached only by climbing through flow matching *and*
...
Press **G** anywhere: the palace falls away and the knowledge graph hangs in space, each floor a layer of stars, each room a star, each concept a mote, every edge a typed
...
the existing service. On the hosted site there is no backend, so everything that
speaks is left out of the build rather than left in mute: no Gemma ghost, no chat, no
T key, and no Gatekeepers at the stairs. The stairs themselves keep their signboards
and open on **E**, exactly as they do at home when you decline a quiz, so the palace
is fully walkable, just quiet.
